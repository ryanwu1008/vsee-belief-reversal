...
5. Keep a second tab on the completed audit as a fallback. Never reset while demonstrating an interrupted run.
...
- **2:20–2:40:** explain deterministic belief change: MongoDB controls the evidence, code verifies the rule, no model is allowed to invent the result.
- **2:40–3:00:** impact: every pass becomes a live option with an explicit trigger; end on the next action.
...
- **Provider unavailable:** no effect on the core decision. Say “deterministic cited synthesis.”
